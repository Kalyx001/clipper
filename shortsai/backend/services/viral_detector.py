...
SYSTEM_PROMPT = f"""You are an expert short-form video editor who has cut thousands of viral \
...
Each clip must:
- Be between {MIN_CLIP_SECONDS} and {MAX_CLIP_SECONDS} seconds long.
...
def detect_viral_moments(segments: list[dict], video_duration: float) -> list[dict]:
    ...
    system = SYSTEM_PROMPT.format(n_min=n_min, n_max=n_max)
