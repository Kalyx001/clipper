...
from config import UPLOADS_DIR, YOUTUBE_COOKIES_PATH
...
def download_youtube(url: str, job_id: str) -> Path:
    ...
    if YOUTUBE_COOKIES_PATH.exists():
        cmd += ["--cookies", str(YOUTUBE_COOKIES_PATH)]
    cmd.append(url)
    ...
    if result.returncode != 0:
        ...
        if "Sign in to confirm" in result.stderr or "not a bot" in result.stderr:
            hint = (
                "\n\nYouTube is blocking this server's IP. Upload a cookies.txt "
                "exported from a logged-in browser session via the app's "
                "'Advanced' section (or POST it to /api/settings/youtube-cookies), "
                "then try again."
            )
        raise RuntimeError(f"yt-dlp failed: {result.stderr[-2000:]}{hint}")
